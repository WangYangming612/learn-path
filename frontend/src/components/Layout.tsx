...
const AppLayout: React.FC = () => {
  ...
  const menuItems: MenuProps["items"] = [
    ...
    { key: "daily", icon: <CalendarOutlined />, label: <Tooltip title="Step 11 开放"><span className="nav-soon">今日学习</span></Tooltip>, disabled: true },
  ...
  return (
    <AntLayout className="app-layout">
      <Sider collapsible collapsed={collapsed} trigger={null} width={232} className="app-sider" breakpoint="lg" onBreakpoint={(broken) => setCollapsed(broken)}>
        ...
        <Menu mode="inline" selectedKeys={[selectedKey]} items={menuItems} className="app-sider__menu" />
      </Sider>

      <AntLayout>
        <Header className="app-header" style={{ background: themeToken.colorBgContainer }}>
          <button type="button" className="app-header__trigger" onClick={() => setCollapsed((v) => !v)} aria-label={collapsed ? "展开侧栏" : "收起侧栏"}>
            {collapsed ? <MenuUnfoldOutlined /> : <MenuFoldOutlined />}
          </button>
          <div className="app-header__right">
            <Dropdown menu={{ items: userMenu }} placement="bottomRight">
              <button type="button" className="app-header__user">
                <Avatar size={36} style={{ background: "linear-gradient(135deg, #0f766e, #115e59)", fontWeight: 600 }}>{initial}</Avatar>
